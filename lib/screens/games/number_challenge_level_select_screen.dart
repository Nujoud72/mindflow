...
import 'number_challenge_screen.dart';

class NumberChallengeLevelSelectScreen extends StatefulWidget {
  const NumberChallengeLevelSelectScreen({super.key});

  @override
  State<NumberChallengeLevelSelectScreen> createState() =>
      _NumberChallengeLevelSelectScreenState();
}

class _NumberChallengeLevelSelectScreenState extends State<NumberChallengeLevelSelectScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    final stars = <int, int>{};
    for (int i = 1; i <= 15; i++) {
      stars[i] = prefs.getInt('numberchallenge_stars_level_$i') ?? 0;
    }
    setState(() {
      _unlockedLevel = prefs.getInt('numberchallenge_unlocked_level') ?? 1;
      _levelStars = stars;
    });
  }
              ...
              onTap: isUnlocked
                  ? () async {
                      await Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => NumberChallengeScreen(startLevel: level)),
                      );
                      _loadProgress();
                    }
                  : null,
