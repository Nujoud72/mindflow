...
import 'word_color_screen.dart';

class WordColorLevelSelectScreen extends StatefulWidget {
  const WordColorLevelSelectScreen({super.key});

  @override
  State<WordColorLevelSelectScreen> createState() =>
      _WordColorLevelSelectScreenState();
}

class _WordColorLevelSelectScreenState extends State<WordColorLevelSelectScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    final stars = <int, int>{};
    for (int i = 1; i <= 15; i++) {
      stars[i] = prefs.getInt('wordcolor_stars_level_$i') ?? 0;
    }
    setState(() {
      _unlockedLevel = prefs.getInt('wordcolor_unlocked_level') ?? 1;
      _levelStars = stars;
    });
  }
              ...
              onTap: isUnlocked
                  ? () async {
                      await Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => WordColorScreen(startLevel: level)),
                      );
                      _loadProgress();
                    }
                  : null,
                ...
                child: Column(
                  ...
                  children: [
                    if (!isUnlocked)
                      ...
                      Text(
                        '$level',
                        style: const TextStyle(color: navyDark, fontSize: 18, fontWeight: FontWeight.bold),
                      ),
                      if (isCompleted) ...[
