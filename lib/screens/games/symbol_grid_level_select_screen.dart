...
import 'symbol_grid_screen.dart';

class SymbolGridLevelSelectScreen extends StatefulWidget {
  const SymbolGridLevelSelectScreen({super.key});

  @override
  State<SymbolGridLevelSelectScreen> createState() =>
      _SymbolGridLevelSelectScreenState();
}

class _SymbolGridLevelSelectScreenState extends State<SymbolGridLevelSelectScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    final stars = <int, int>{};
    for (int i = 1; i <= 15; i++) {
      stars[i] = prefs.getInt('symbolgrid_stars_level_$i') ?? 0;
    }
    setState(() {
      _unlockedLevel = prefs.getInt('symbolgrid_unlocked_level') ?? 1;
      _levelStars = stars;
    });
  }
              ...
              onTap: isUnlocked
                  ? () async {
                      await Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => SymbolGridScreen(startLevel: level)),
                      );
                      _loadProgress();
                    }
                  : null,
