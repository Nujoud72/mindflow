...
import 'dot_connect_screen.dart';

class DotConnectLevelSelectScreen extends StatefulWidget {
  const DotConnectLevelSelectScreen({super.key});

  @override
  State<DotConnectLevelSelectScreen> createState() =>
      _DotConnectLevelSelectScreenState();
}

class _DotConnectLevelSelectScreenState extends State<DotConnectLevelSelectScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    final stars = <int, int>{};
    for (int i = 1; i <= 15; i++) {
      stars[i] = prefs.getInt('dotconnect_stars_level_$i') ?? 0;
    }
    setState(() {
      _unlockedLevel = prefs.getInt('dotconnect_unlocked_level') ?? 1;
      _levelStars = stars;
    });
  }
              ...
              onTap: isUnlocked
                  ? () async {
                      await Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => DotConnectScreen(startLevel: level)),
                      );
                      _loadProgress();
                    }
                  : null,
